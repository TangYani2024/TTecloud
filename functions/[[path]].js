...
function rSP(f, o, p, cfg) {
  ...
  '    if (b) b.style.width = p + "%"; if (s) s.innerHTML = "<img class=\\\'om-emoji\\\' src=\\\'/openmoji/26A1.svg\\\' alt=\\\'⚡\\\'> " + (p >= 100 ? "拼装 Blob 中..." : "多线程接收中..."); if (spd) spd.innerText = sp + " MB/s"; if (by) by.innerText = formatBytes(db) + " / " + formatBytes(FILE.size); if (pc) pc.innerText = p + "%";' +
  '  };' +
  ...
  '    const sp = el > 0 ? (db / 1048576 / el).toFixed(1) : "0.0";' +
  '    const b = document.getElementById("dl-bar"), s = document.getElementById("dl-status"), spd = document.getElementById("dl-speed"), by = document.getElementById("dl-bytes"), pc = document.getElementById("dl-pct");' +
  '    if (b) b.style.width = p + "%"; if (s) s.innerHTML = "<img class=\\\'om-emoji\\\' src=\\\'/openmoji/26A1.svg\\\' alt=\\\'⚡\\\'> " + (p >= 100 ? "正在刷盘固化..." : "流式直写磁盘中..."); if (spd) spd.innerText = sp + " MB/s"; if (by) by.innerText = formatBytes(db) + " / " + formatBytes(FILE.size); if (pc) pc.innerText = p + "%";' +
  '  };' +
  '  const tasks = Array.from({ length: tc }, (_, i) => {' +
  '    const s = i * cs, e = Math.min(s + cs - 1, FILE.size - 1);' +
  ...
  '  const c = document.getElementById("share-action-container"); if (!c) return;' +
  '  c.innerHTML = \'<div style="width:100%;padding:14px;background:rgba(0,0,0,.03);border-radius:12px;border:1px inset var(--cd);text-align:left"><div style="text-align:center;margin-bottom:10px"><h4 style="margin:0;font-size:15px;display:flex;align-items:center;justify-content:center;gap:6px"><img class="om-emoji om-emoji-lg" src="/openmoji/1F680.svg" alt="🚀"> 超大文件加速下载指引</h4><p style="color:gray;font-size:12px;margin:6px 0 0">当前文件大小为 <b style="color:var(--primary,#3b82f6)">\' + formatBytes(FILE.size) + \'</b>（大于 500MB）</p></div><div style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.25);padding:10px 12px;border-radius:10px;margin-bottom:10px;font-size:12px;color:#065f46;display:flex;align-items:center;gap:8px"><img class="om-emoji" src="/openmoji/2705.svg" alt="✅"><span><b>已自动将高速直链复制到剪贴板！</b></span></div><p style="font-size:12px;color:gray;margin:0 0 10px;line-height:1.5">当前浏览器环境暂不支持本地磁盘流式直写，直接读入内存会导致网页闪退。强烈建议粘贴剪贴板直链至专业多线程工具下载：</p><div style="display:flex;flex-direction:column;gap:6px;margin-bottom:12px;font-size:12px"><div class="app-badge-item">💻 <b>Windows / Mac</b>: IDM / Motrix / FDM / 迅雷</div><div class="app-badge-item">📱 <b>手机端 (iOS / Android)</b>: IDM+ / 迅雷 / 闪电下载</div></div><div style="display:flex;flex-direction:column;gap:8px"><button type="button" class="btn btn-success btn-sm" style="width:100%;padding:10px" onclick="copyTxt(FILE.url);alert(\\\'已重新复制直链！\\\')"><img class="om-emoji" src="/openmoji/1F4CB.svg" alt="📋"> 再次复制直链</button><a href="\' + FILE.url + \'" class="btn btn-outline btn-sm" target="_blank" style="text-decoration:none;padding:10px;text-align:center"><img class="om-emoji" src="/openmoji/1F4E5.svg" alt="📥"> 仍尝试浏览器原生下载</a><button type="button" class="btn btn-outline btn-sm" style="padding:8px" onclick="restoreShareActions()">返回</button></div></div>\';' +
  '  alert("✅ 超大文件高速直链已自动复制到剪贴板！");' +
  '}' +
  '</script></body></html>';
}
